...
export default function AddProductModal({ isOpen, onClose }: AddProductModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    onClose();
    setFormData({ name: "", category: "", unit: "bt", cost_price: 0, min_threshold: 0 });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <Card className="w-full max-w-md bg-card border-muted/30">
        <div className="p-4 border-b border-muted/20 flex justify-between items-center">
          <h2 className="text-xl font-bold text-white">Nuovo Prodotto</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <Input 
              ...
              placeholder="es. Gin, Vodka, Tonica..."
            />
          </div>
